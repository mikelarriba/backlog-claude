    ...
    try {
      ...
      if (!Array.isArray(docs) || !docs.length) {
        return sendError(
          ...
          400,
          'VALIDATION_ERROR',
          'docs array is required and must not be empty'
        );
      }
    ...
    try {
      ...
      if (!Array.isArray(orderedFilenames) || !orderedFilenames.length) {
        return sendError(
          ...
          400,
          'VALIDATION_ERROR',
          'orderedFilenames array is required and must not be empty'
        );
      }
          ...
          try {
            if (!item.filename || !item.docType || typeof item.rank !== 'number') {
              skipped.push({
                ...
                reason: 'missing filename, docType, or rank',
              });
              return;
            }
      ...
      for (const entry of assignments) {
        if (
          ...
          return sendError(
            ...
            400,
            'VALIDATION_ERROR',
            'Each assignment must have docType, filename, and sprint as strings'
          );
        }
      }
