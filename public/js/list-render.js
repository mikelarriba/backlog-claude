...
export function buildTreeOrder(docs, collapsed = _collapsedItems) {
    ...
    function place(doc, indent) {
        if (placed.has(key(doc)))
            return;
        placed.add(key(doc));
        ordered.push({ doc, indent });
        if (collapsed.has(doc.filename))
    ...
    docs.forEach((d) => {
        if (!d.parentFilename || !byFilename.has(d.parentFilename))
            place(d, 0);
    });
...
export function renderSwimlaneSectionHtml(sectionKey, label, versionName, docs) {
    ...
    if (sectionKey !== 'backlog') {
        ...
        const options = versions
            .map((v) => `<option value="${escHtml(v.name)}" ${v.name === versionName ? 'selected' : ''}>${escHtml(v.name)}${v.released ? ' (released)' : ''}</option>`)
            .join('');
        versionSelector = `
