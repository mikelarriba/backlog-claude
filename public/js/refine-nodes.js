...
import { escHtml, showJiraToast, TYPE_LABEL, postJSON, fetchJSON, deleteJSON, getErrorMessage, } from './state.js';
...
export async function _fpMoveToEpic(filename, docType, fromEpic, toEpic, featureFilename) {
    try {
        ...
        if (doc)
            upsertDoc({ ...doc, parentFilename: toEpic });
...
export function _openCellCreateForm(type, col, row, epicFilename, epicDocType) {
    ...
    document
        ...
        ?.addEventListener('click', () => _executeEmptyCellCreate(type, col, row, epicFilename, epicDocType));
...
export async function _executeEmptyCellCreate(type, col, row, epicFilename, epicDocType) {
    ...
    try {
        ...
        if (epicDocType === 'epic')
            genBody.parentEpic = epicFilename;
        if (epicDocType === 'feature')
            genBody.parentFeature = epicFilename;
...
export function _showMultiCardContextMenu(x, y, epicFilename, docType) {
    ...
    popup
        ...
        ?.addEventListener('click', () => _moveCardsToEdge([..._canvasSelectedCards], 'left', epicFilename, docType));
    popup
        .querySelector('#_ctx-m-right')
        ?.addEventListener('click', () => _moveCardsToEdge([..._canvasSelectedCards], 'right', epicFilename, docType));
    popup
        .querySelector('#_ctx-m-top')
        ?.addEventListener('click', () => _moveCardsToEdge([..._canvasSelectedCards], 'top', epicFilename, docType));
    popup
        .querySelector('#_ctx-m-bottom')
        ?.addEventListener('click', () => _moveCardsToEdge([..._canvasSelectedCards], 'bottom', epicFilename, docType));
    popup.querySelector('#_ctx-m-delete')?.addEventListener('click', async () => {
        _closeLinkPopup();
        if (!confirm(`Delete ${count} selected items? This cannot be undone.`))
            return;
...
export async function _moveCardsToEdge(filenames, direction, epicFilename, docType) {
    _closeLinkPopup();
    const layout = _activePanelState.layout;
...
export async function _executeCanvasSplit(originalFilename, childDocType, epicFilename, epicDocType) {
    ...
    try {
        ...
        const { content: origContent } = (await fetchJSON(`/api/doc/${childDocType}/${encodeURIComponent(originalFilename)}`));
        ...
        if (origDoc?.fixVersion)
            genBody.fixVersion = origDoc.fixVersion;
        if (origDoc?.pi && origDoc.pi !== 'TBD')
            genBody.pi = origDoc.pi;
        if (epicDocType === 'epic')
            genBody.parentEpic = epicFilename;
        if (epicDocType === 'feature')
            genBody.parentFeature = epicFilename;
...
//# sourceMappingURL=refine-nodes.js.map