...
export function initRoadmapDragDrop() {
    ...
    cards.forEach((card) => {
        ...
        card.addEventListener('dragover', (e) => {
            ...
            if (zone === 'before')
                card.classList.add('rm-insert-before');
            else
                card.classList.add('rm-insert-after');
        ...
        card.addEventListener('drop', async (e) => {
            ...
            try {
                ...
                if (data.filename === card.dataset['filename'])
                    return;
    ...
    dropZones.forEach((zone) => {
        ...
        zone.addEventListener('dragleave', (e) => {
            const dragEvent = e;
            if (!zone.contains(dragEvent.relatedTarget))
...
export function attachRoadmapDepHoverListeners() {
    document.querySelectorAll('.roadmap-card[data-filename]').forEach((el) => {
        ...
        if (!doc)
            return;
        if (!(doc.blocks || []).length && !(doc.blockedBy || []).length)
            return;
