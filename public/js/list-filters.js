...
import { buildChildrenMap, getDescendants, debounce, escHtml, putJSON, postJSON, showJiraToast, SECTION_LABELS, } from './state.js';
...
export function collapseAll() {
    ...
    for (const d of allDocs) {
        if ((d.docType === 'feature' || d.docType === 'epic') &&
...
export async function updatePiVersion(sectionKey, versionName) {
    ...
    if (sectionKey === 'currentPi')
        update.currentPi = versionName || null;
    if (sectionKey === 'nextPi')
        update.nextPi = versionName || null;
    try {
...
export function applyFilters(..._args) {
    ...
    if (activeTypeFilter !== 'all')
        filtered = filtered.filter((d) => d.docType === activeTypeFilter);
    if (activeStatusFilter !== 'all')
        filtered = filtered.filter((d) => (d.status || 'Draft') === activeStatusFilter);
    if (activeTeamFilter !== 'all')
        filtered = filtered.filter((d) => d.team === activeTeamFilter);
    if (activeWorkCatFilter !== 'all')
        filtered = filtered.filter((d) => d.workCategory === activeWorkCatFilter);
    if (q)
        filtered = filtered.filter((d) => d.title.toLowerCase().includes(q) || d.filename.toLowerCase().includes(q));
...
export function handleItemClick(e, filename, docType) {
    ...
    if (isShift && _lastClickedItem) {
        ...
        const lastIdx = visible.findIndex((v) => v.filename === lastClicked.filename && v.docType === lastClicked.docType);
...
export function showContextMenu(x, y) {
    ...
    for (const [pi, sprints] of Object.entries(sprintConfig)) {
        for (const s of sprints) {
            if (!allSprints.has(s.name))
                allSprints.set(s.name, pi);
        }
    }
    const sprintItems = Array.from(allSprints.entries())
        .map(([name, _pi]) => `<button class="ctx-item" onclick="contextAssignField('sprint','${escHtml(name)}')">${escHtml(name)}</button>`)
        .join('');
    const sprintClear = `<button class="ctx-item" onclick="contextAssignField('sprint','')">Clear sprint</button>`;
    // "Assign Team" submenu
    const teamItems = (_metaTeams || [])
        .map((t) => `<button class="ctx-item" onclick="contextAssignField('team','${escHtml(t)}')">${escHtml(t)}</button>`)
        .join('');
    ...
    const catItems = (_metaWorkCategories || [])
        .map((c) => `<button class="ctx-item" onclick="contextAssignField('workCategory','${escHtml(c)}')">${escHtml(c)}</button>`)
        .join('');
...
function _closeContextMenuHandler(e) {
    if (!e.target.closest('#list-context-menu'))
        closeContextMenu();
}
