...
export async function searchDocumentationIssues() {
    ...
    try {
        ...
        if (_searchText.trim())
            params.set('text', _searchText.trim());
        if (_fixVersionFilter)
            params.set('fixVersion', _fixVersionFilter);
...
export function toggleSuggestionRow(index) {
    if (_expandedSuggestionIndexes.has(index))
        _expandedSuggestionIndexes.delete(index);
    else
        _expandedSuggestionIndexes.add(index);
    const row = document.querySelector(`.doc-suggestion-row[data-index="${index}"]`);
    if (row)
        row.classList.toggle('expanded', _expandedSuggestionIndexes.has(index));
}
export function toggleSuggestionCheck(index, checked) {
    if (checked)
        _selectedSuggestionIndexes.add(index);
    else
        _selectedSuggestionIndexes.delete(index);
    const row = document.querySelector(`.doc-suggestion-row[data-index="${index}"]`);
    if (row)
        row.classList.toggle('selected', checked);
    _updateSuggestionSelectionState();
...
async function executeChanges() {
    if (_selectedSuggestionIndexes.size === 0)
        return;
    ...
    try {
        ...
        if (successCount)
            void logAiSaving('doc_confluence_modify', successCount);
    }
    catch (err) {
...
function _renderDiffHtml(s) {
    ...
    if (!lines.length)
        return '<div class="doc-diff-empty">No content to compare.</div>';
    return lines
        .map((line) => {
