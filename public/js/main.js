...
import { saveTitle, cancelTitleEdit, updateDocStatus, showList, confirmDelete, closeDeleteDialog, executeDelete, toggleDropdown, closeDropdown, closeAllDropdowns, toggleOriginal, openDoc, } from './detail.js';
import { saveStoryPoints, updateDocSprint, updateDocTeam, updateDocWorkCategory, addDocComment, startCommentEdit, cancelCommentEdit, saveCommentEdit, deleteDocComment, } from './detail-fields.js';
import { toggleHierarchy, loadHierarchy, linkExistingChildren, toggleHierarchyChild, } from './detail-links.js';
...
import { jiraSelectAll, jiraSelectCancel, jiraSelectConfirm, searchJira, downloadSelected, pullByKey, toggleJiraItem, } from './jira-import.js';
import { syncPreviewSelectAll, syncPreviewCancel, syncPreviewConfirm, pushToJira, } from './jira-push.js';
import { pullFromJira, checkAllJira, submitUpdateFromJiraKey } from './jira-pull.js';
import { openBugForm, closeBugForm, onBugFilesSelected, submitBugReport, removeBugFile, } from './bugcreate.js';
...
import { togglePiConfigSection, addSprintRow, removeSprintRow, selectPiConfigTab, saveSprintConfig, saveSplitThreshold, loadAllSprintConfigs, renderPiConfigTabs, _updatePiFromConfig, syncPiFromJira, confirmJiraSprintImport, skipJiraSprintImport, dismissJiraImportBanner, } from './piconfig.js';
import { openDistributionModal, closeDistributionModal, applyDistribution, } from './distribution.js';
import { openRoadmapView, closeRoadmapView, refreshRoadmapView, toggleRoadmapPi, toggleRoadmapPanel, filterRoadmapEpics, focusEpic, addDepLink, addParallelLink, removeDepLink, closeDepModal, closeSplitModal, executeSplit, } from './roadmap.js';
import { pushSprintsToJira, closeSprintPushModal, toggleSprintPushFilter, sprintPushSelectAll, sprintPushToggleAllSprints, startSprintPushPreview, confirmSprintPush, _sprintPushUpdateCount, pullFromJiraSprints, closePullSprintModal, pullSprintToggleAll, startPullSprintPreview, pullSprintSelectAllItems, _pullSprintUpdateCount, confirmPullSprint, } from './roadmap-jira-sync.js';
import { handleEpicContextMenu, handleStoryContextMenu, rmCtxOpenEpic, rmCtxMoveEpic, rmCtxMoveStory, rmCtxSetSprint, } from './roadmap-context-menus.js';
import { handleRoadmapCardClick, handleRoadmapEpicClick, clearRoadmapSelection, } from './roadmap-select.js';
import { loadSkillsView, toggleSkillCard, saveSkill, resetSkill, improveSkill, saveProductContext, resetProductContext, } from './skills.js';
import { initDragDrop } from './dragdrop.js';
import { toggleModelSection, loadModelSetting, onProviderChange, refreshProviders, updateModelSetting, updateEffortSetting, } from './provider-settings.js';
import { _connectSSE } from './sse-client.js';
import { toggleAiSavingsSection, loadAiSavingsSection, filterAiSavings, exportAiSavingsPdf, exportAiSavingsPptx, } from './ai-savings.js';
import { loadBugsDashboard, refreshBugsDashboard, filterBugsTable, analyzeBugs, closeBugsAnalysis, bugToggleKey, bugToggleAll, toggleClosedBugs, } from './bugs-dashboard.js';
import { loadDocumentationView, docFilterInput, docSetTypeFilter, docSetFixVersion, searchDocumentationIssues, docSetPage, docRowClick, docToggleKey, askAI, toggleSuggestionRow, toggleSuggestionCheck, selectAllSuggestions, deselectAllSuggestions, modifyDocumentation, undoChanges, } from './documentation.js';
...
export function highlightSelectedItem(filename, docType) {
    ...
    if (filename) {
        document
            .querySelector(`.epic-item[data-filename="${CSS.escape(filename)}"][data-doctype="${docType}"]`)
            ?.classList.add('selected');
        document
            .querySelector(`.roadmap-card[data-filename="${CSS.escape(filename)}"][data-doctype="${docType}"]`)
            ?.classList.add('selected');
    }
}
...
function navigateTo(viewName) {
    ...
    if (fabContainer)
        fabContainer.style.display = viewName === 'backlog' ? '' : 'none';
...
function _renderTeamFilterPills(teams) {
    ...
    if (!container)
        return;
    container
        .querySelectorAll('[data-team]:not([data-team="all"])')
