...
import { escHtml, showJiraToast, TYPE_LABEL, streamSSE, stripFrontmatter, patchJSON, postJSON, fetchJSON, deleteJSON, getErrorMessage, renderMarkdown, } from './state.js';
import { loadDocs } from './list.js';
import { buildCanvasGraph, renderCanvas, rebuildCanvasEdges, _renderFpCanvas, computeAutoLayout, } from './refine-canvas.js';
...
export async function openManualRefine(filename, docType) {
    ...
    if (titleEl)
        titleEl.textContent = doc?.title || filename;
    // Switch views
...
export function closeRefineView() {
    ...
    if (canvas)
        canvas.classList.remove('manage-links-active');
...
export async function renderFeatureMultiPanel(featureFilename) {
    ...
    for (const epic of data.epics) {
        ...
        if (ps)
            _renderFpCanvas(epic.filename, ps, featureFilename);
    }
}
...
export function _toggleEpicPanel(epicFilename, featureFilename) {
    ...
    if (chevron)
        chevron.textContent = panel.classList.contains('fp-collapsed') ? '▶' : '▼';
...
export async function openRefinePanel(filename, docType) {
    ...
    try {
      ...
      <div class="rp-comments-section comments-section hidden" id="rp-comments-section"></div>`;
        ...
        _renderComments(_parseComments(content), filename, docType, document.getElementById('rp-comments-section'));
    }
    catch {
...
async function _loadRpDeps(filename, docType) {
    ...
    if (!section)
        return;
    try {
        const data = (await fetchJSON(`/api/links/${encodeURIComponent(docType)}/${encodeURIComponent(filename)}`));
...
export async function saveRpTitle() {
    ...
    if (!newTitle || newTitle === original || !filename)
        return;
    try {
...
export async function saveRpStoryPoints(filename, docType) {
    ...
    try {
        ...
        if (spEl)
            spEl.textContent = newVal ? `${newVal} SP` : '';
        // Update in-memory
        const doc = allDocs.find((d) => d.filename === filename && d.docType === docType);
        if (doc)
            doc.storyPoints = newVal === '' ? null : Number(newVal);
    }
    catch {
        input.value = orig;
    }
...
export async function saveRpPriority(filename, docType) {
    ...
    try {
        ...
        showJiraToast('error', `Failed to save priority: ${e instanceof Error ? e.message : String(e)}`);
    }
}
...
export async function executeRpUpgrade(filename, docType) {
    ...
    try {
        let result = null;
        await streamSSE(`/api/doc/${docType}/${encodeURIComponent(filename)}/upgrade`, { feedback }, {
            onText: (text) => {
        ...
        if (result) {
            ...
            if (rpContent)
                rpContent.innerHTML = renderMarkdown(stripFrontmatter(content));
            await loadDocs();
            ...
            if (updated) {
                const card = document.querySelector(`.canvas-card[data-filename="${CSS.escape(filename)}"] .canvas-card-title`);
                if (card)
                    card.textContent = updated.title;
            }
        }
...
export async function executeRpCreate(type) {
    ...
    try {
        ...
        if (parentDoc?.fixVersion)
            genBody.fixVersion = parentDoc.fixVersion;
        if (parentDoc?.pi && parentDoc.pi !== 'TBD')
            genBody.pi = parentDoc.pi;
        if (_canvasDocType === 'epic')
            genBody.parentEpic = _canvasEpicFilename ?? undefined;
        if (_canvasDocType === 'feature')
            genBody.parentFeature = _canvasEpicFilename ?? undefined;
