...
import { _showEpicContextMenu, _showEmptyCellMenu, _showCardContextMenu, _showMultiCardContextMenu, _showFpCardContextMenu, } from './refine-nodes.js';
...
export function _renderFpCanvas(epicFilename, ps, featureFilename) {
    ...
    for (const c of ps.stories)
        positions[c.filename] = ps.layout[c.filename] || { col: 0, row: 0 };
...
export function computeAutoLayout(children, blocks, _parallel) {
    ...
    for (const { src, tgt } of blocks) {
        if (!blockedByMap.has(tgt))
            blockedByMap.set(tgt, []);
    ...
    for (const child of children)
        colSets.set(child.filename, child.filename);
    function findRoot(fn) {
        if (colSets.get(fn) === fn)
            return fn;
...
export function renderCanvas(epicFilename, docType) {
    ...
    const usedCols = [...new Set(Object.values(layoutEntries).map((p) => p.col))].sort((a, b) => a - b);
    const usedRows = [...new Set(Object.values(layoutEntries).map((p) => p.row))].sort((a, b) => a - b);
    if (usedCols.length || usedRows.length) {
        ...
        for (const fn of Object.keys(layoutEntries)) {
            ...
            if (newCol !== layoutEntries[fn].col || newRow !== layoutEntries[fn].row)
                changed = true;
            layoutEntries[fn] = { col: newCol, row: newRow };
        }
        if (changed)
            saveCanvasLayout(_activePanelState, epicFilename);
    }
    ...
    for (let row = 0; row < gridRows; row++) {
        for (let col = 0; col < gridCols; col++) {
            ...
            cell.addEventListener('drop', async (e) => {
                ...
                const cur = layoutEntries[fn] || {};
                if (cur.col === newCol && cur.row === newRow)
                    return;
                layoutEntries[fn] = { col: newCol, row: newRow };
    ...
    for (const child of _activePanelState.stories) {
      ...
      <div class="canvas-handle canvas-handle--right"  data-side="right"></div>`;
        ...
        card.addEventListener('click', (e) => {
            if (e.target.classList.contains('canvas-handle'))
                return;
...
function drawCanvasEdges(svg, cardPositions, _epicFilename, _epicCenterX, _totalW) {
    ...
    for (const colItems of Object.values(byCols)) {
        ...
        for (let i = 0; i < colItems.length - 1; i++) {
            ...
            if (!src || !tgt || src === tgt)
                continue;
            const hasBlocks = blocksList.some((b) => (b.src === colItems[i].fn && b.tgt === colItems[i + 1].fn) ||
