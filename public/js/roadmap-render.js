...
export function renderEpicPanel(sprints) {
  ...
  body.querySelectorAll('.rm-epic-reorder-handle').forEach((handle) => {
    const card = handle.closest('.rm-epic-card');
...
function refocusEpicHandle(filename) {
  setTimeout(() => {
    document
      .querySelector(
...
// resulting order.
async function moveEpicRank(filename, docType, action) {
...
export function buildEpicReorderHandleHtml(filename, title) {
  if (!filename) return '';
...
// reads the same way.
export function buildEpicMoveAnnouncement(title, action, moved) {
