...
export function renderEpicPanel(sprints) {
    ...
    const visibleLeafs = allDocs.filter((d) => leafTypes.has(d.docType) && d.fixVersion && _roadmapVisiblePis.has(d.fixVersion));
...
export function renderStoryPanel(sprints) {
    ...
    const piDocs = allDocs.filter((d) => leafTypes.has(d.docType) && d.fixVersion && _roadmapVisiblePis.has(d.fixVersion));
...
export function renderRoadmapCard(d, _sprintName) {
    ...
    if (blocks.length)
        depHtml += `<div class="dep-badge dep-blocks">→ blocks ${blocks.length}</div>`;
    if (parallel.length)
...
export function showFeatureTooltip(e) {
    ...
    if (desc)
        html += `<div class="rm-tooltip-desc">${escHtml(desc)}</div>`;
...
export function injectGhostCards() {
    ...
    const crossPiStories = allDocs.filter((d) => {
        if (!leafTypes.has(d.docType) || !d.parentFilename || !d.fixVersion)
            return false;
