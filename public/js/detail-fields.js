...
export function _renderComments(comments, filename, docType, containerEl) {
...
export function computeChildPoints(filename, docType) {
    ...
    const children = allDocs.filter((d) => {
        if (docType === 'feature')
            return d.docType === 'epic' && d.parentFilename === filename;
        if (docType === 'epic')
            return ((d.docType === 'story' || d.docType === 'spike' || d.docType === 'bug') &&
    ...
    for (const c of children) {
        if (docType === 'feature') {
            ...
            const epicChildren = allDocs.filter((d) => (d.docType === 'story' || d.docType === 'spike' || d.docType === 'bug') &&
                d.parentFilename === c.filename);
            for (const ec of epicChildren)
                sum += Number(ec.storyPoints) || 0;
        }
        else {
...
export async function saveStoryPoints() {
    ...
    if (newVal === orig || !currentFilename || !currentDocType)
        return;
    try {
        ...
        if (doc)
            upsertDoc({ ...doc, storyPoints: newVal === '' ? null : Number(newVal) });
    }
    catch {
        input.value = orig;
    }
...
export function updateSprintSelect(docType, fixVersion, currentSprint) {
    ...
    sel.innerHTML =
        '<option value="">No Sprint</option>' +
            sprints
                .map((s) => `<option value="${escHtml(s.name)}"${s.name === currentSprint ? ' selected' : ''}>${escHtml(s.name)}</option>`)
                .join('');
...
export async function updateDocSprint(sprint) {
    ...
    try {
        ...
        const doc = allDocs.find((d) => d.filename === currentFilename && d.docType === currentDocType);
        if (doc)
            upsertDoc({ ...doc, sprint: sprint || null });
    }
    catch (e) {
...
export function updateTeamWorkCatSelects(doc) {
    ...
    document.getElementById('detail-workcat-select').value =
        doc?.workCategory || '';
}
...
export async function updateDocWorkCategory(workCategory) {
    ...
    try {
        ...
        const doc = allDocs.find((d) => d.filename === currentFilename && d.docType === currentDocType);
        if (doc)
            upsertDoc({ ...doc, workCategory: workCategory || null });
    }
    catch (e) {
