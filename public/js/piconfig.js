...
export function renderPiConfigTabs() {
    ...
    const versionOptions = (versions || [])
        .map((v) => `<option value="${escHtml(v.name)}">${escHtml(v.name)}${v.released ? ' (released)' : ''}</option>`)
        .join('');
    ...
    if (pis.length && !_piConfigActivePi)
...
function _renderPiTabButtons() {
    ...
    if (piSettings.nextPi)
        pis.push({ key: 'nextPi', label: 'Next PI', name: piSettings.nextPi });
    if (!pis.length)
        return '';
    return pis
        .map((p) => `<button class="pi-config-tab${_piConfigActivePi === p.name ? ' active' : ''}"
...
export async function syncPiFromJira(sectionKey) {
    ...
    try {
        ...
        const selected = await showJiraSelectModal(`${issues.length} JIRA issue(s) for fix version "${versionName}"`, items, 'Import selected', '✓ Already imported');
        if (!selected.length)
            return;
...
export function confirmJiraSprintImport() {
    if (!_piConfigActivePi || !_jiraImportCandidates.length)
        return;
...
export async function saveSprintConfig() {
    ...
    try {
        const data = (await putJSON(`/api/settings/pi/sprints/${encodeURIComponent(_piConfigActivePi)}`, {
            sprints,
        }));
