...
export function contextSplitItem() {
    ...
    modal.querySelector('#issue-split-title').textContent =
...
export async function executeSplitIssue() {
    ...
    try {
        ...
        const { content: origContent } = (await fetchJSON(`/api/doc/${docType}/${encodeURIComponent(filename)}`));
        ...
        if (origDoc?.parentFilename) {
            ...
            if (parentDoc?.docType === 'epic')
                genBody.parentEpic = origDoc.parentFilename;
            if (parentDoc?.docType === 'feature')
                genBody.parentFeature = origDoc.parentFilename;
        }
