...
import { fetchJSON, postJSON, stripFrontmatter, escHtml, showJiraToast, toggleSection, TYPE_LABEL, STATUS_LABEL, renderMarkdown, } from './state.js';
...
export function renderDetailDeps(doc) {
    ...
    for (const fn of blockedBy)
        chips.push(depChip(fn, 'dep-chip-blocked', '🔒', 'blockedBy'));
    for (const fn of blocks)
        chips.push(depChip(fn, 'dep-chip-blocks', '→', 'blocks'));
    for (const fn of parallel)
        chips.push(depChip(fn, 'dep-chip-parallel', '#', 'parallel'));
...
export async function loadHierarchy(filename, docType) {
    ...
    try {
      ...
      <div class="hierarchy-child"
        ...
        if (parent)
            rows.push(makeParentRow(parent));
        for (const child of children)
            rows.push(makeChildRow(child));
        const parts = [];
        if (parent)
            parts.push(`↑ ${TYPE_LABEL[parent.docType]}`);
        if (children.length)
            parts.push(`↓ ${children.length} linked`);
...
export async function linkExistingChildren() {
    if (!currentFilename || (currentDocType !== 'epic' && currentDocType !== 'feature'))
        return;
    ...
    try {
        ...
        for (const c of linkData.children || [])
            linkedFilenames.add(c.filename);
    }
    catch (e) {
    ...
    const selected = await showJiraSelectModal(`Link existing ${childLabel(currentDocType)} to "${allDocs.find((d) => d.filename === currentFilename)?.title || currentFilename}"`, candidates, 'Link selected');
    if (!selected.length)
        return;
    const linkedItems = [];
    ...
    if (linkedItems.length > 0) {
        ...
        for (const item of linkedItems) {
            ...
            if (doc)
                upsertDoc({ ...doc, parentFilename: currentFilename });
        }
