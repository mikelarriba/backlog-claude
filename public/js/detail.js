// ── Detail view ────────────────────────────────────────────────
import { fetchJSON, patchJSON, deleteJSON, postJSON, stripFrontmatter, escHtml, showJiraToast, toggleSection, TYPE_LABEL, renderMarkdown, openModal, closeModal, } from './state.js';
...
import { updateStoryPointsUI, updateSprintSelect, updateTeamWorkCatSelects, _renderComments, _parseComments, } from './detail-fields.js';
...
export async function openDoc(filename, docType) {
    ...
    try {
        ...
        if (docType === 'epic' || docType === 'feature')
            loadHierarchy(filename, docType);
        else
    ...
    if (currentDocType === 'epic' || currentDocType === 'feature') {
        try {
            ...
            if (children.length) {
                ...
                const selected = await showJiraSelectModal(`Delete "${title}" and ${children.length} child item${children.length !== 1 ? 's' : ''}?`, items, 'Delete selected');
                // User cancelled
                if (!selected.length && !confirm(`Delete only "${title}" without its children?`))
                    return;
