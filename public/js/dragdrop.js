...
import { buildChildrenMap, getDescendants, postJSON, showJiraToast, TYPE_LABEL, DRAG_TARGETS, SECTION_LABELS, } from './state.js';
...
export function getSwimlaneSection(doc) {
    ...
    if (doc.fixVersion && piSettings.currentPi && doc.fixVersion === piSettings.currentPi)
        return 'currentPi';
    if (doc.fixVersion && piSettings.nextPi && doc.fixVersion === piSettings.nextPi)
        return 'nextPi';
    return 'backlog';
}
export function sectionToFixVersion(section) {
    if (section === 'currentPi')
        return piSettings.currentPi;
    if (section === 'nextPi')
        return piSettings.nextPi;
...
async function executeDropLink() {
    if (!_pendingDropSrc || !_pendingDropTgt)
        return;
...
async function executeDropDep() {
    ...
    try {
        ...
        if (srcDoc) {
            ...
            if (!srcDoc.blocks.includes(tgt.filename))
                srcDoc.blocks.push(tgt.filename);
        }
        const tgtDoc = allDocs.find((d) => d.filename === tgt.filename);
        if (tgtDoc) {
            tgtDoc.blockedBy = tgtDoc.blockedBy || [];
            if (!tgtDoc.blockedBy.includes(src.filename))
                tgtDoc.blockedBy.push(src.filename);
        }
        applyFilters();
...
export function hideInsertionMarker() {
    if (_insertionMarker)
        _insertionMarker.style.display = 'none';
}
...
function computeInsertBefore(srcDocType, clientY) {
    const items = [...document.querySelectorAll('#epic-list .epic-item')].filter((el) => el.dataset.doctype === srcDocType && !el.classList.contains('drag-source'));
    ...
    if (snap.started && snap.ghost) {
        ...
        if (!dropTarget) {
            ...
            if (sectionUnder) {
                ...
                if (sectionUnder.dataset.section !== srcLane)
                    dropSwimlane = sectionUnder;
            }
        }
    }
    return {
...
export function initDragDrop() {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        if (!state.started) {
            ...
            if (multiCount > 1) {
                ...
                ghost.appendChild(document.createTextNode(allDocs.find((d) => d.filename === state.srcFilename)?.title || state.srcFilename));
            }
            ...
            if (multiCount > 1) {
                ...
                if (srcItem)
                    srcItem.classList.add('drag-source');
            }
    ...
    document.addEventListener('mouseup', async (e) => {
        ...
        if (!snap.started)
            return;
        if (dropTarget)
            return showDropActionPopup(snap.srcFilename, snap.srcDocType, dropTarget, e.clientX, e.clientY);
        if (dropSwimlane)
            return executeMoveDrop(snap.srcFilename, snap.srcDocType, dropSwimlane);
        if (snap.isReranking)
