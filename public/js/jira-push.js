...
export function showSyncPreviewModal(title, items, confirmLabel) {
    return new Promise(function (resolve) {
        ...
        const parts = [];
        if (createCount)
            parts.push(`${createCount} new`);
        if (updateCount)
            parts.push(`${updateCount} update`);
        if (deleteCount)
            parts.push(`${deleteCount} to delete`);
                ...
                changesHtml =
                    '<div class="sync-preview-changes">' +
                        item.changes
                            .map(function (c) {
                            if (c.field === 'error')
                                return ('<div class="sync-preview-change"><span class="sync-preview-field">error</span><span class="sync-preview-to" style="color:var(--error-text)">' +
                                    escHtml(c.message || '') +
                                    '</span></div>');
                            if (c.field === 'description')
                                return ('<div class="sync-preview-change"><span class="sync-preview-field">description</span><span class="sync-preview-to">' +
                            ...
                            return ('<div class="sync-preview-change"><span class="sync-preview-field">' +
                                escHtml(c.field) +
                                '</span>' +
                                fromHtml +
                                toHtml +
...
export function syncPreviewSelectAll(checked) {
    document
        .querySelectorAll('#sync-preview-list .sync-preview-cb')
        .forEach(function (cb) {
        cb.checked = checked;
    });
    _syncPreviewUpdateCount();
...
export function syncPreviewConfirm() {
    const selected = Array.from(document.querySelectorAll('#sync-preview-list .sync-preview-cb:checked')).map(function (cb) {
...
export function finishJiraProgress(summaryText, hasError) {
    ...
    const labelEl = document.getElementById('sync-progress-label');
    if (labelEl)
        labelEl.textContent = hasError ? 'Finished with errors' : 'All done ✅';
    const summary = document.getElementById('sync-progress-summary');
    if (summary) {
...
export async function pushToJira() {
    ...
    const selected = await showSyncPreviewModal('↑ Push to JIRA — Preview', previewItems, `Push ${previewItems.length} item${previewItems.length !== 1 ? 's' : ''}`);
    if (!selected) {
        updateJiraPushBtn();
        return;
    }
    ...
    const pushParts = [];
    if (created)
        pushParts.push(`${created} created`);
    if (updated)
        pushParts.push(`${updated} synced`);
    if (errorMessages.length)
        pushParts.push(`${errorMessages.length} failed`);
    if (currentFilename)
    ...
    if (storyResults.length) {
        void logAiSaving('story_push', storyResults.length, storyResults.map((r) => r.key));
    }
    if (spikeResults.length) {
        void logAiSaving('spike_push', spikeResults.length, spikeResults.map((r) => r.key));
    }
}
