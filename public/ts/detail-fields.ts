...
export function _serializeComments(comments: Comment[]): string {
...
export function computeChildPoints(
  filename: string,
  docType: string,
  docs: DocEntry[]
): number | null {
...
export function updateStoryPointsUI(docType: string, sp: number | null): void {
  ...
  if (isLeaf) {
    ...
    const sum = computeChildPoints(currentFilename as string, docType, allDocs);
