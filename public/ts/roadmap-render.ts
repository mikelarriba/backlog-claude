...
export function spCardHeight(sp: number): number {
  const n = Number(sp) || 0;
...
// For epics that carry an "Estimated Sprint Size" (1–4) but aren't refined
// into stories yet, the roadmap shows N striped placeholder cards — one per
// estimated sprint — so a rough roadmap can be laid out before refinement.
// Each placement is persisted on the epic (estimatedSprints); the rest sit in
// the Unassigned column until dragged into a sprint.
export interface EstPlaceholder {
  epicFilename: string;
  epicTitle: string;
  color: string;
  fromSprint: string; // sprint name, or '' for the Unassigned column
}
...
export function buildEstPlaceholders(
  epics: DocEntry[],
  knownSprintNames: Set<string>
): Map<string, EstPlaceholder[]> {
  const map = new Map<string, EstPlaceholder[]>();
  const push = (key: string, p: EstPlaceholder): void => {
...
export function buildEstPlaceholderCardHtml(p: EstPlaceholder): string {
  return `
...
export function updateEstPlacements(
  placements: string[],
  size: number,
...
export function visibleEstimateEpics(): DocEntry[] {
...
export function renderEpicPanel(sprints: RoadmapSprint[]): void {
  ...
  for (const [key, { epicDoc, sprints: sprintSet, storyCount, totalSP }] of sorted) {
    ...
    const spanSprints = new Set<string>([...sprintSet, ...estSprints]);
...
export function buildRoadmapCardHtml(
  d: DocEntry,
  parent: { title: string; workCategory?: string | null } | null | undefined
): string {
