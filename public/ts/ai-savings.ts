...
export async function loadSidebarSavings(): Promise<void> {
  ...
  try {
    ...
    console.warn('Failed to load sidebar AI savings:', (e as Error).message);
  }
}

let _sidebarChart: ChartInstance | null = null;

function renderSidebarSavings(entries: AiSavingsEntry[]): void {
...
function _renderSidebarChart(entries: AiSavingsEntry[]): void {
  const canvas = document.getElementById('sidebar-savings-chart') as HTMLCanvasElement | null;
