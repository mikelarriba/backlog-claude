...
describe('proposeDistribution — single sprint', () => {
  test('places docs that fit within capacity', () => {
    const docs = [
      makeDoc({ filename: 'a.md', storyPoints: 3 }),
...
describe('proposeDistribution — dependency ordering', () => {
  test('blocked doc is placed in a later sprint than its blocker', () => {
    ...
    const blockerSprint = result.sprints.findIndex((s) =>
      s.assigned.some((d) => d.filename === 'blocker.md')
    );
    const blockedSprint = result.sprints.findIndex((s) =>
      s.assigned.some((d) => d.filename === 'blocked.md')
    );
    assert.ok(
      blockedSprint > blockerSprint,
      `blocked (sprint ${blockedSprint}) should come after blocker (sprint ${blockerSprint})`
    );
    void placement; // suppress unused warning
  });

  test('blocker already assigned: respects its sprint when placing the blocked doc', () => {
    const blocker = makeDoc({
      filename: 'blocker.md',
      storyPoints: 3,
      sprint: 'S1',
      blocks: ['blocked.md'],
    ...
    const blockedBucket = result.sprints.find((s) =>
      s.assigned.some((d) => d.filename === 'blocked.md')
    );
...
describe('proposeDistribution — parallel co-location', () => {
  test('parallel siblings are placed in the same sprint when possible', () => {
    ...
    assert.ok(
      inS1.includes('a.md') && inS1.includes('b.md'),
      'both parallel stories should be in S1'
    );
  });
...
describe('proposeDistribution — buffer', () => {
  test('respects bufferPct on sprint config', () => {
    ...
    const result = proposeDistribution(
      docs,
      [{ name: 'S1', capacity: 10, bufferPct: 0.2 }],
      EMPTY_EPIC_MAP
    );
    assert.equal(result.sprints[0].assigned.length, 1);
  ...
  test('effectiveCapacity is floor of capacity * (1 - bufferPct)', () => {
    ...
    const result = proposeDistribution(
      docs,
      [{ name: 'S1', capacity: 15, bufferPct: 0.3 }],
      EMPTY_EPIC_MAP
    );
